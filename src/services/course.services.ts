...
import { Course, CourseCreate, CourseRead, CourseResult, UserCourseResult } from "../interfaces";
import { client } from "../database";
import { readCourseSchema, courseSchema } from "../schemas";
import { AppError } from "../errors";

const create = async ( payload: CourseCreate, admin: boolean ): Promise<Course> => {
  if ( !admin ) {
...
const read = async (): Promise<CourseRead> => {
  ...
  const coursesReturn = readCourseSchema.parse(courses);
...
const register = async ( admin: boolean, courseId: string, userId: string ): Promise<{message: string}> => {
  ...
  const queryResult: UserCourseResult = await client.query(queryFormat);
